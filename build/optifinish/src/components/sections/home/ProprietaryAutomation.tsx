...
export default function ProprietaryAutomation() {
  ...
  // Independent carousel index per product
  const [imgIndexes, setImgIndexes] = useState([0, 0, 0]);

  const product = PRODUCTS[active];

  // Auto-switch product every 6 s
  useEffect(() => {
    const t = setInterval(() => {
  ...
  // Each product's carousel advances independently every 3 s
  useEffect(() => {
    const intervals = PRODUCTS.map((_, i) =>
      ...
      }, 3000 + i * 300) // slight offset so they don't all flip at once
    );
  ...
  return (
    <section className="relative bg-[#f1efea] py-16 md:py-24">
      ...
      <div className="relative mx-auto w-full max-w-7xl px-5 md:px-8">
        ...
        {/* ══════════════════════════════════════════
            DESKTOP layout — two-col, timer-driven (hidden lg:grid)
        ══════════════════════════════════════════ */}
        <div className="hidden lg:grid lg:grid-cols-2 lg:items-center lg:gap-16">
          ...
          <div className="flex flex-col">
            ...
            <span className="mb-3 block text-[0.56rem] font-bold uppercase tracking-[0.22em] text-ink/40">
              OptiFinish Automation
            </span>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="overflow-hidden rounded-[1.6rem] border border-ink/[0.07] bg-white/70 shadow-[0_8px_32px_rgba(0,0,0,0.08)]"
            >
              <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#f0f0f0]">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={product.id + '-desk-img-' + currentImgIndex}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                ...
                <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2">
                  {product.images.map((_, i) => (
                    <button
                      ...
                      onClick={() => setImgIndexes(prev => { const n = [...prev]; n[active] = i; return n; })}
                      className={`rounded-full transition-all duration-300 ${
                        i === currentImgIndex ? 'h-1.5 w-6 bg-white' : 'h-1.5 w-1.5 bg-white/40 hover:bg-white/65'
                      }`}
                    />
                  ))}
                </div>
